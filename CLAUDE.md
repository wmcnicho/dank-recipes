...
**PWA** (, , icons in , generated — cream bg + fire emoji, opaque/unrounded per iOS rules): installable via Share → Add to Home Screen. All PWA paths account for the  Pages subpath (//uid=501(hunter) gid=20(staff) groups=20(staff),12(everyone),61(localaccounts),79(_appserverusr),80(admin),81(_appserveradm),701(com.apple.sharepoint.group.1),33(_appstore),98(_lpadmin),100(_lpoperator),204(_developer),250(_analyticsusers),395(com.apple.access_ftp),398(com.apple.access_screensharing),399(com.apple.access_ssh),400(com.apple.access_remote_ae) hardcode it; icon + SW paths are relative / ). The SW pre-caches the shell and hashed assets at install (parsed from index.html), network-first navigations, stale-while-revalidate assets, skips cross-origin (recipe images, Apps Script). Registered in , production builds only. GitHub Pages serves everything with ; SW update checks bypass HTTP cache, so deploys land within ~10 min.
